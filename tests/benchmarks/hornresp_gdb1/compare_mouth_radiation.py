...
def main():
    ...
    with open(REPO / "tests/benchmarks/hornresp_gdb1/hornresp_params_hirob.txt") as f:
        params = f.read()
    
    print("Hornresp params (key values):")
    for line in params.splitlines():
        if any(k in line for k in ["S1", "S2", "AT", "Vrc", "Lrc", "Sd", "Ang", "End Correction"]):
            print(f"  {line.strip()}")
    
    print("\n=== Test 1: n_segments convergence ===")
    for n_seg in [50, 100, 126, 200, 400]:
        f, r = run_benchmark(n_segments=n_seg, mouth_rad="anechoic", 
                            driver_overrides=dict(spl_response=None, lossy_le=False))
        log_f = np.log10(f)
        log_hr = np.log10(hr_freqs)
        py_spl = interp1d(log_f, r.spl_power_based, kind='linear', fill_value='extrapolate')(log_hr)
        valid = (hr_freqs >= f.min()) & (hr_freqs <= f.max())
        d = py_spl[valid] - hr_spls[valid]
        print(f"  n_seg={n_seg:4d}: mean={d.mean():+6.2f}  std={d.std():5.2f}  "
    ...
    for mouth_rad in ["anechoic", "levine"]:
        f, r = run_benchmark(n_segments=126, mouth_rad=mouth_rad,
                            driver_overrides=dict(spl_response=None, lossy_le=False))
        log_f = np.log10(f)
        log_hr = np.log10(hr_freqs)
        py_spl = interp1d(log_f, r.spl_power_based, kind='linear', fill_value='extrapolate')(log_hr)
        ...
        for lo, hi in [(10, 100), (100, 1000), (1000, 5000), (5000, 10000)]:
            ...
            if m.sum() > 0:
                print(f"    {lo:4d}-{hi:5d} Hz: mean={d[m].mean():+6.2f}  std={d[m].std():5.2f}")
    
    print("\n=== Test 3: Compare at key frequencies ===")
    f, r = run_benchmark(n_segments=126, mouth_rad="anechoic",
                        driver_overrides=dict(spl_response=None, lossy_le=False))
    log_f = np.log10(f)
    log_hr = np.log10(hr_freqs)
    py_pb = interp1d(log_f, r.spl_power_based, kind='linear', fill_value='extrapolate')(log_hr)
    
    f2, r2 = run_benchmark(n_segments=126, mouth_rad="levine",
                           driver_overrides=dict(spl_response=None, lossy_le=False))
    py_pb2 = interp1d(np.log10(f2), r2.spl_power_based, kind='linear', fill_value='extrapolate')(log_hr)
    
    print(f"  {'Freq':>7} | {'Hornresp':>8} | {'anechoic':>8} | {'levine':>8} | {'Δ anechoic':>9} | {'Δ levine':>8}")
    for target in [20, 30, 50, 80, 100, 200, 500, 1000, 2000, 3000, 5000, 8000, 10000]:
        i = np.argmin(np.abs(hr_freqs - target))
        print(f"  {hr_freqs[i]:7.1f} | {hr_spls[i]:8.2f} | {py_pb[i]:8.2f} | {py_pb2[i]:8.2f} | "
    ...
    ax = axes[1]
    valid = (hr_freqs >= f.min()) & (hr_freqs <= f.max())
    ax.plot(hr_freqs[valid], py_pb[valid] - hr_spls[valid], "r-", lw=1.0, label="Δ anechoic", alpha=0.7)
    ax.plot(hr_freqs[valid], py_pb2[valid] - hr_spls[valid], "g-", lw=1.0, label="Δ levine", alpha=0.7)
    ...
    plt.tight_layout()
    out = REPO / "tests/benchmarks/hornresp_gdb1/hirob_mouth_rad_comparison.png"
