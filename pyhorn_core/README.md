...
| `fold` | `pyhorn_fold` (extracted) | SLSQP-based horn path optimisation |
| `segment` | `pyhorn_segment` (extracted) | Medial-axis computation and auto-segment pipeline |
| `wavefront` | `pyhorn_wavefront` (extracted) | 2-D direct Helmholtz solver, PML, time domain |
...
`pyhorn_core` owns the `physics` engine (TMM with Miki 1990 frequency-dependent absorption) and the surrounding numerics (`solver/`). The three sibling packages (`pyhorn_fold`, `pyhorn_wavefront`, `pyhorn_registry`) are standalone dependencies that `pyhorn_core` links against.

## Extracted Packages

| Package | One-line description | Hornresp manual |
|---------|----------------------|-----------------|
| `pyhorn_fold` | SLSQP-optimised horn path folding — given a straight-axis target path and cross-section, computes a physical folded geometry that fits a rectangular cabinet | §Folding / §Staircase |
| `pyhorn_wavefront` | 2-D rectangular Helmholtz solver — direct sparse matrix, optional PML absorbing boundary, time-domain variant, mouth polar directivity | §Wave |
...
These packages live in the same repository at `/Users/guillaume/P/GdB1/` and are installed as regular dependencies of `pyhorn_core`.
...
horn   = parse_horn_geometry("source/bk16.yaml")
result = horn_response(driver, horn, fmin=20, fmax=20000)
...
horn = parse_horn_geometry("source/bk16_mk1.yaml")
